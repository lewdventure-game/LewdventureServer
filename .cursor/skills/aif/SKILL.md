...
python3 ~/.cursor/skills/aif-skill-generator/scripts/security-scan.py <installed-skill-path>
```
...
For each recommended skill:
  ...
  3. SECURITY: Scan installed EXTERNAL skill (never built-in aif*) → run the selected concrete Python command with `security-scan.py <path>`
     - BLOCKED? → run the selected concrete Python command with `cleanup-blocked-skill.py --skill <name> --installed-path <path>` (reuse the same <path> from step 3, NOT a synthesized .cursor/skills/<name>), warn user, skip this skill
...
```bash
node ~/.cursor/skills/aif/references/update-config.mjs \
  --template ~/.cursor/skills/aif/references/config-template.yaml \
...
3. Run `node ~/.cursor/skills/aif/references/update-config.mjs --template ~/.cursor/skills/aif/references/config-template.yaml --target .ai-factory/config.yaml --payload .ai-factory/config.update.json`
...
7. For each external skill from skills.sh:
   ...
   python3 ~/.cursor/skills/aif-skill-generator/scripts/security-scan.py <installed-path>
   ```
   - Exit 1 (BLOCKED) → run the selected concrete Python command with `~/.cursor/skills/aif-skill-generator/scripts/cleanup-blocked-skill.py --skill <name> --installed-path <installed-path>` (reuse the same `<installed-path>` you passed to security-scan.py — upstream `skills` sanitizes the directory name, so synthesizing it from `<name>` can miss the real folder), warn user, skip this skill
...
9. Configure MCP in `.cursor/mcp.json`
...
AI Factory writes MCP config to `.cursor/mcp.json`, but the outer settings shape depends on the runtime.
...
4. **MCP in `.cursor/mcp.json`** — Project-level MCP configuration
