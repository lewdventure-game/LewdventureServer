...
Пауза `statuses_cooldown` (Constants) между срабатываниями и после последнего (статус wait статус wait). Пустая очередь — без wait. Нет маппера статуса в очереди / нет `statuses_cooldown` → ERROR (не silent skip и не Wait(0) вместо константы).
